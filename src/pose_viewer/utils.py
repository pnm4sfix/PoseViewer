...
def extract_bouts(viewer, h5_files, fps = 330., window_denominator = 8, activity_threshold = 2, confidence_threshold = 0.8):

    #viewer = Viewer([], n_nodes = 19, center_node = 13)
    zebdata = ZebData()

    viewer.fps = fps
    viewer.tsne_window = 2 * int(viewer.fps/window_denominator) # this was changed to 2x
    ...
    for h5_file in h5_files:
        ...
        for key in viewer.coords_data.keys():
                    ...
                    # calculate movement
                    viewer.calulate_orthogonal_variance(amd_threshold = activity_threshold,
                    ...
                    file_bout_index = pd.DataFrame([np.arange(N), [h5_file] * N, all_starts, all_ends]).T
...
def extract_bouts_from_filedict(viewer, file_dict, fps = 330., window_denominator = 8, activity_threshold = 2, confidence_threshold = 0.8):
    ...
    for fish, (h5_file, log_file, protocol_dict) in file_dict.items():
        ...
        for key in viewer.coords_data.keys():
                    ...
                    for n,  (bhv_start, bhv_end) in enumerate(viewer.behaviours):
                        ...
                        new_start = int(bhv_mid - int(T/2))
                        new_end = int(bhv_mid + int(T/2))
                        new_end = (T- (new_end-new_start)) + new_end # what is the point in this again
                        bhv = points[:, new_start:new_end]
                        ci = cis[:, new_start:new_end]
                        ci = ci.reshape((*ci.shape, 1))
                    ...
                    for protocol, (start, end) in protocol_dict.items():
                        ...
                        file_bout_index.loc[start_filter & end_filter, "protocol"] = protocol
                    #file_bout_index.reset_index(inplace = True)
                    bout_map = pd.concat([bout_map, file_bout_index])
...
def classification_data_to_bouts(classification_data, C, T, V, M, fps, denominator, center = None):
...
class Animation(object):
    ...
    def __init__(self, dataset, skeleton, label_dict = None, shuffle = True, normalise = False):
        super().__init__()
        self.dataloader = torch.utils.data.DataLoader(dataset, batch_size=8, shuffle=shuffle) 
    ...
    def plot_graph(self):
        ...
        cmap = plt.cm.plasma
        self.ax.cla()
        self.nodes = nx.draw_networkx_nodes(self.G, pos = self.pos, node_color=node_colors, cmap = cmap, node_size = 20, ax = self.ax)
    ...
    def setup(self):
        if self.normalise:
            ...
            self.behaviours[:, 0] = (self.behaviours[:, 0] - self.behaviours_x_mean) / self.behaviours_x_std
            self.behaviours[:, 1] = (self.behaviours[:, 1] - self.behaviours_y_mean) / self.behaviours_y_std
    ...
    def normalise_bhv(self):
        ...
        self.bhv[0] = (self.bhv[0] - self.behaviours_x_mean) / self.behaviours_x_std
        self.bhv[1] = (self.bhv[1] - self.behaviours_y_mean) / self.behaviours_y_std
    ...
    def update(self, frame):
        ...
        for bhv in range(self.behaviours.shape[0]):
            ...
            for previous_frame in range(frame):
                self.ax.plot(self.bhv[0, previous_frame], self.bhv[1, previous_frame], color = "gray", linewidth = 0.3, alpha = 0.5)
    ...
    def plot_kde(self, frame):
        self.x, self.y = np.swapaxes(self.bhv_type[:, :2, frame], 0, 1).reshape(2, -1)
        self.ax.cla()
        sns.kdeplot(
            x=self.x, y=self.y, fill=True,
             gridsize = 500, 
                levels = 15,
                thresh = 0.1,
                ax = self.ax)
    ...
    def animate(self, repeat = False):
        ...
        self.fig, self.axes = plt.subplots(ncols = self.dataloader.batch_size, figsize = (20, 10))
        self.ani = FuncAnimation(self.fig, self.update, frames=self.behaviours.shape[2], blit=True, repeat=repeat, interval = 1000/10, init_func = self.setup)
        #display(HTML(self.ani.to_jshtml()))
        
    def animate_kde(self):
        self.fig, self.axes = plt.subplots(ncols = np.unique(self.dataset.labels).shape[0], figsize = (40, 10))
        self.ani = FuncAnimation(self.fig, self.update_kde, frames=20, blit=True, repeat=True, interval = 1000/10, init_func = self.setup_kde)
        display(HTML(self.ani.to_jshtml()))
