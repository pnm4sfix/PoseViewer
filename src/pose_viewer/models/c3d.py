...
class C3D(LightningModule):
    def __init__(self, num_class,
        ...
        self.conv1a = self._conv_layer_set(self.num_channels, 32, kernel_size, padding)    #ConvModule(3, 64, **c3d_conv_param)
        self.pool1 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))

        self.conv2a = self._conv_layer_set(32, 64, kernel_size, padding) #ConvModule(64, 128, **c3d_conv_param)
        self.pool2 = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2))

        self.conv3a = self._conv_layer_set(64, 128, kernel_size, padding)#ConvModule(128, 256, **c3d_conv_param)
        self.conv3b = self._conv_layer_set(128, 256, kernel_size, padding)#ConvModule(256, 256, **c3d_conv_param)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_loss", loss, prog_bar = True, sync_dist = True, logger = True, on_epoch = True)
    ...
    def on_train_start(self):
        ...
        self.logger.log_hyperparams(self.hparams, {"hp/learning_rate": self.learning_rate, "hp/batch_size": self.batch_size})
    ...
    def setup(self, stage = None):
        ...
        if stage == "predict":
            ...
            target_transform = Lambda(lambda y: torch.zeros(self.num_classes, dtype=torch.float).scatter_(0, torch.tensor(y), value=1))
            ...
            if stage == "fit" or stage is None:
                ...
                self.pose_train, self.pose_val = Subset(self.train_data, train_idx), Subset(self.train_data, val_idx)
            
 
                if self.ideal_sample_no is not None:
                    print('Dynamically Augmenting DataSet')
                    # create two new blank ZebData
                    self.pose_train_data = ZebData(ideal_sample_no = self.ideal_sample_no) # this will be augmented
                    self.pose_val_data = ZebData()

                    # assign data and labels to these new ones using train and val indices
                    self.pose_train_data.data = self.pose_train.dataset.data[self.pose_train.indices]
                    self.pose_train_data.labels = self.pose_train.dataset.labels[self.pose_train.indices]

                    self.pose_val_data.data = self.pose_val.dataset.data[self.pose_val.indices]
                    self.pose_val_data.labels = self.pose_val.dataset.labels[self.pose_val.indices]
    ...
    def train_dataloader(self):
        return DataLoader(self.pose_train, batch_size = self.batch_size, num_workers = self.num_workers, pin_memory = True, shuffle = True)
    
    def val_dataloader(self):
        return DataLoader(self.pose_val, batch_size = self.batch_size, num_workers = self.num_workers, pin_memory = True)
    
    def test_dataloader(self):
        return DataLoader(self.pose_test, batch_size = self.batch_size, num_workers = self.num_workers, pin_memory = True)
